...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!verifyPassword(password)) {
      await audit('admin.login_failed', {}, { ip: req.headers.get('x-forwarded-for') || '' });
      return NextResponse.json({ ok: false, message: 'Incorrect password.' }, { status: 401 });
    }
    const session = await createSession(req.headers.get('x-forwarded-for') || '');
    await audit('admin.login_success', {});
