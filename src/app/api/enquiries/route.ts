...
const ALLOWED_AUDIENCES = ['buyer', 'developer', 'cp'] as const;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const audience: Audience = (ALLOWED_AUDIENCES as readonly string[]).includes(body.audience)
      ? body.audience
      : 'buyer';
    ...
    const project_name = sanitizeText(body.project_name || '', 200);
    const unit_count = sanitizeText(body.unit_count || '', 40);
    const rera_number = sanitizeText(body.rera_number || '', 80);

    if (full_name.length < 2)
      return NextResponse.json({ ok: false, message: 'Enter your name.' }, { status: 400 });
    if (!/^[6-9]\d{9}$/.test(mobile))
      return NextResponse.json({ ok: false, message: 'Enter a valid 10-digit mobile.' }, { status: 400 });
    ...
    await db.execute({
      sql: `INSERT INTO enquiries
        (audience, full_name, email, mobile, city, configuration, budget_range, message, source_page,
         company_name, designation, project_name, unit_count, rera_number)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        audience,
        full_name,
    ...
    await audit(
      'enquiry.submitted',
      { audience, full_name, mobile, city },
      { ip: req.headers.get('x-forwarded-for') || '' }
    );

    void sendToCustomerOrAdmin('admin', null, {
      title: `New ${audience} enquiry from ${full_name}`,
      body: `${mobile}${city ? ' · ' + city : ''}${configuration ? ' · ' + configuration : ''}${budget_range ? ' · ' + budget_range : ''}${company_name ? ' · ' + company_name : ''}`,
      url: '/admin',
      tag: 'enquiry-new',
    }).catch(() => {});
